...
- **Streaming topology & live health** — discover streaming tables and trace their source→sink flow as an interactive DAG, with ingestion-source classification (Kafka / Kinesis / Event Hub / Auto Loader / Delta), producing-pipeline mapping, and freshness SLA badges (fresh / lagging / stale). A live metrics dashboard adds per-pipeline update status, success rate, throughput, backlog, and data-quality with trend sparklines — sourced from `system.lakeflow.pipeline_update_timeline` + DLT `flow_progress` events, all fail-open. (`GET /api/lineage/streaming-topology`, `GET /api/lineage/streaming-metrics`)
...
- **Column Transformation panel — tabs + AI overview + deep framework analysis (v2.6.x)** — the panel is organized into **Columns / Analyze / History / Producers** tabs. An **AI overview** modal gives a plain-English summary of a table's transformations plus a per-column explanation with a source→transform→target graphic (`POST /api/column-transformations/overview`). For metadata-driven (config-table) ETL frameworks where direct analysis finds no column logic, a **deep framework analysis** runs an agentic, streaming second pass — detect config mechanism → read parameters → query config tables → derive columns — narrating each step (`POST /api/column-transformations/deep-analyze`, `backend/server/framework_analysis.py`).
- **Business view (v2.6.x)** — a **Technical ⇄ Business** toggle on the lineage canvas flips the detailed engineering graph into a plain-language view for non-engineers: relabels technical types to business terms (Process, Data pipeline, Dataset, Live dataset, File…), humanizes `snake_case` names (`orders_curated` → "Orders Curated"), shows a one-line plain-English description per dataset, and hides fully-qualified names, column detail, IDs, cost, and health. Client-side and instant; the preference persists. (`frontend/src/lib/businessView.ts`)
- **Data-only vs Data + processing + precise dataset lineage (v2.6.x)** — a business-view sub-toggle shows just the datasets (and how they connect) or the datasets plus the jobs/pipelines that move data between them. Data-only renders the **real per-row `(source → target)` pairs** from `system.access.table_lineage` (`table_edges` on `GET /api/lineage/trace`) instead of reconstructing edges by cross-producting an entity's inputs × outputs — which fixes the "everything connected to everything" mesh on heavily-connected hub tables.
- **AI "Explain this lineage" (v2.6.x)** — a lightbulb in business view opens a modal with an AI-generated, plain-English explanation of the *current* on-screen graph: an overall summary plus an ordered "source → process → output" walkthrough. It respects the current view (data-only vs data+processing) and requires a reachable Foundation Model endpoint. (`POST /api/lineage/explain`)
