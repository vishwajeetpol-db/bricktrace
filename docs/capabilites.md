...
| Streaming topology & live health (v2.6.x) | Tabbed **Topology (DAG) / Metrics** screen: discovers streaming tables + their source→sink flow (interactive reactflow DAG), classifies the ingestion source (Kafka/Kinesis/Event Hub/Auto Loader/Delta), maps each to its producing pipeline, and shows a freshness SLA badge. The Metrics tab adds per-pipeline update status, success rate, throughput, backlog, and data-quality with trend sparklines. Fuzzy catalog picker + 30s auto-refresh. All metrics fail open. | `backend/routes/capability_closures.py` (`streaming_topology`, `streaming_metrics`), `/api/lineage/streaming-topology`, `/api/lineage/streaming-metrics`, `StreamingTopologyPanel.tsx`, `graph/StreamTopologyGraph.tsx` — see [capability_code_map.md](capability_code_map.md) Part J |
