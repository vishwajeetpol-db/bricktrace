...
- **Streaming Topology — observability screen (Tiers 1–3).** The discovery MVP became a tabbed **Topology (DAG) / Metrics** view. `GET /api/lineage/streaming-topology` is enriched (per-stream ingestion-source classification, producing pipeline id+name, freshness SLA bucket) and a new `GET /api/lineage/streaming-metrics` adds per-pipeline update status/success-rate/duration (`system.lakeflow.pipeline_update_timeline`) plus throughput/backlog/data-quality from DLT `flow_progress` events (pipeline-events REST, short-TTL cached). Both in `backend/routes/capability_closures.py`; frontend `StreamingTopologyPanel.tsx` + reactflow `components/graph/StreamTopologyGraph.tsx`. All metrics **fail open**.
- **Deterministic transformation re-parse fix** — `PARSER_VERSION` (`transformation_lineage/versioning/change_detection.py`) bumped 5→6. The change-detection token folds `PARSER_VERSION` in and the pipeline skips unchanged artifacts; an earlier parser improvement had shipped without bumping the key, so improved logic never re-ran on unchanged notebooks (`lineage_edge_endpoints` stayed empty and the panel looped on "Generate"). The bump forces a one-time re-parse. See §4.3.
...
| GET | `/api/lineage/streaming-topology?catalog=` | **(2.6.x)** Streaming tables + source→sink edges, **enriched**: per-stream `source_kind` (Kafka/Kinesis/Event Hub/Auto Loader/Delta), producing `pipeline_id`+`pipeline_name` (`system.lakeflow.pipelines`), and `freshness` SLA bucket from `last_altered` |
| GET | `/api/lineage/streaming-metrics?pipeline_ids=` | **(2.6.x)** Per-pipeline live metrics — update status/success-rate/duration (`system.lakeflow.pipeline_update_timeline`) + throughput/backlog/data-quality from DLT `flow_progress` events (pipeline-events REST, short-TTL cache). Fails open |
