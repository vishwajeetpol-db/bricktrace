...
def deep_analyze_stream(
    ...
    for t in tables:
        ...
        # rather than marking it unreadable and deriving nothing.
        ...
        if res is None:
            ...
            detail = f" ({str(err)[:120]})" if err else " (invalid or unreadable)"
        ...
        if res.get("total_rows", 0) == 0:
            ...
            # `certain: false` means detection only GUESSED this name — an empty
            # guess proves nothing. But if we RESOLVED it to a real table above,
            # the table provably exists, so its emptiness is meaningful.
            if t.get("certain") is False and used == name:
                uncertain_empty.append(used)
