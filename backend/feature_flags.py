...
FLAG_DEFINITIONS: list[dict] = [
    ...
    },
    {
        ...
        "description": (
            ...
            "from a secret scope (never stored in the registry). Entitlement (§7.6): when "
            "ENFORCE_USER_IDENTITY is on, the requesting user is checked against the TARGET table "
            "(fails closed); when it's off, the account SP fetches on behalf of any authorized app "
            "user (accepted trust boundary). When this flag is off — or no peer is registered — a "
            "cross-workspace producer returns the honest 409 from Phase 0 and analysis degrades to "
            "captured plans / config tables. See docs/FEDERATED_LINEAGE_DESIGN.md §7."
        ),
        "cost": "low",
        "risk": "high",
        "side_effects": [
            "The account SP reads producer source in a PEER workspace — a real cross-workspace data path. With ENFORCE_USER_IDENTITY on it's gated per requesting user; with it off any authorized app user can trigger it (SP trust boundary). Review §7.6 before enabling.",
